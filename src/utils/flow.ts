...
const sendFlowMessage = async (
  ...
  if (component) messageToSend = { ...messageToSend, components: [component] };
...
const proceedFlow = async (
  flow: Flow,
  channel: Channel,
  data?: { messageData?: MessageData; messageSubType?: string }
) => {
...
const deleteLastMessageWithTimeout = (flow: Flow) =>
  setTimeout(async () => {
